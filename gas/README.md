...
5. 在 Apps Script 上方函式選單選擇 `setupProject`，執行一次並完成 Sheet 與 Google Calendar 授權。
...
前端右下角的「日曆」按鈕會開啟嵌入式 Google 日曆，並提供「訂閱到我的 Google 日曆」。
...
```json
{"status":"success","schemaVersion":"3","events":[]}
```
...
7. 建立一般未過期活動，確認 `calendarEventId` 有值，Google 日曆中出現同名行程。
