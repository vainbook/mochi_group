...
固定團功能需要表格結構版本 `2`。新版 GAS 貼上後一定要重新執行一次 `setupProject()`；表格尚未更新時，網頁會顯示但停用「固定團（需更新）」，避免資料寫進舊表格後遺失。當 `/exec` 回傳 `schemaVersion: "2"` 後就會自動啟用。
...
管理員可以刪除任何活動，但仍只有活動目前主揪可以編輯內容或交棒。請不要把管理員密碼分享給不需要此權限的成員。
